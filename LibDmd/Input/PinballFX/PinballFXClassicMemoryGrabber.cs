...
{
	/// <summary>
	/// Polls for the Pinball FX Classic process, grabs the DMD display from the memory.
	/// </summary>
	/// <remarks>
	/// Can be launched any time. Will wait with sending frames until Pinball FX Classic is
	/// launched and stop sending when it exits.
	/// </remarks>
	public class PinballFXClassicMemoryGrabber : MemoryGrabber<DmdFrame>, IGray2Source, IDmdColorSource, IGameNameSource
	{
		...
		private void ReadGameName(IntPtr hProcess)
		{
			...
			if (!identical) {
				Logger.Info($"Found Pinball FX Classic game: {name}");
